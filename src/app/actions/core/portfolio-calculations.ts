// src/app/actions/core/portfolio-calculations.ts
// Pure portfolio calculation functions - no external data dependencies

import { Balance } from "snaptrade-typescript-sdk";
...
export function calculateTotalBalance(holdingsData: HoldingsData): number {
  ...
  return cash + equities + options + crypto + other;
}
...
export function calculateTotalReturn(holdingsData: HoldingsData, realizedPnL: number = 0) {
  const currentValue = calculateTotalBalance(holdingsData);
  const unrealizedPnL = calculateTotalUnrealizedPnL(holdingsData);
  const totalPnL = unrealizedPnL + realizedPnL;
  const costBasis = currentValue - unrealizedPnL; // Original cost basis of current holdings
  ...
  return {
    ...
    totalReturnPercentage: costBasis !== 0 ? (totalPnL / costBasis) * 100 : 0
  };
}
